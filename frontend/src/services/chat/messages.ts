...
export class MessageProcessor {
  ...
  private static processAssistantMessage(data: SDKMessage, messageId: string, timestamp: number): ProcessedMessage {
    ...
    const thinkingContent = thinkingBlocks.map(block => (block as any).thinking || block.text).join('') || '';
    ...
    // Check if this is a synthetic message (generated by the bridge service)
    const isSynthetic = this.isSyntheticMessage(messageId, displayContent);
   ...
   * Check if this is a synthetic message generated by the bridge service
   */
  private static isSyntheticMessage(messageId: string, content: string): boolean {
