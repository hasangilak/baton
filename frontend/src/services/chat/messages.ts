...
export class MessageProcessor {
  ...
  private static processAssistantMessage(data: SDKMessage, messageId: string, timestamp: number): ProcessedMessage {
    ...
    const thinkingContent = thinkingBlocks.map(block => block.thinking || block.text).join('') || '';
    
    // Prioritize thinking content for status messages, fall back to text content
    const displayContent = thinkingContent || textContent;
    
    // Check if this is a status/transient message
    const isTransient = this.isStatusMessage(displayContent) || thinkingBlocks.length > 0;
    ...
    if (toolUseBlocks.length > 0) {
      ...
      return {
        id: messageId,
        type: 'tool',
        content: displayContent || `Using ${toolUseBlocks[0]?.name}`,
        timestamp,
        metadata: {
    ...
    // Regular assistant message (including thinking messages)
    return {
      id: messageId,
      type: 'assistant',
  ...
  private static isStatusMessage(content: string): boolean {
    ...
    const statusPatterns = [
      ...
      /^I\s+need\s+to\s+understand/i, // I need to understand...
      /^Looking\s+at\s+the\s+code/i, // Looking at the code...
      /^Let\s+me\s+examine/i,   // Let me examine...
      /^I\s+should\s+check/i,   // I should check...
      /^I\s+can\s+help\s+you\s+with/i, // I can help you with...
      /^I'll\s+search\s+for/i,  // I'll search for...
      /^I'll\s+help\s+you/i,    // I'll help you...
      /^Let\s+me\s+start\s+by/i, // Let me start by...
      /^I'll\s+begin\s+by/i,    // I'll begin by...
      /^Working\s+on/i,         // Working on...
      /^Examining\b/i,          // Examining...
      /^Reviewing\b/i,          // Reviewing...
      /^Checking\b/i,           // Checking...
      // Synthetic status message patterns
      /^Searching\s+the\s+web\s+for/i, // Searching the web for...
      /^Reading\s+[\w.-]+\.\.\./i,     // Reading filename...
      /^Editing\s+[\w.-]+\.\.\./i,     // Editing filename...
      /^Writing\s+to\s+[\w.-]+\.\.\./i, // Writing to filename...
      /^Executing:\s+/i,               // Executing: command...
      /^Running\s+agent\s+task:/i,     // Running agent task:...
      /^Searching\s+for\s+files/i,     // Searching for files matching:...
      /^Searching\s+for:\s+/i,         // Searching for: pattern
      /^Listing\s+contents\s+of/i,     // Listing contents of...
      /^Fetching\s+content\s+from/i,   // Fetching content from...
      /^Updating\s+todo\s+list/i,      // Updating todo list...
      /^Navigating\s+to\s+/i,          // Navigating to...
      /^Clicking\s+on\s+/i,            // Clicking on...
      /^Typing\s+text/i,               // Typing text...
      /^Taking\s+screenshot/i,         // Taking screenshot...
      /^Capturing\s+page\s+snapshot/i, // Capturing page snapshot...
      /^Searching\s+GitHub\s+for/i,    // Searching GitHub for:...
      /^Using\s+[\w\s]+\.\.\./i,       // Using tool...
    ];
