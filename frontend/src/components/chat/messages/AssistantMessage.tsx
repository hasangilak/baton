...
import { SyntheticMessageLoader } from '../shared/SyntheticMessageLoader';
...
export const AssistantMessage: React.FC<AssistantMessageProps> = ({
  ...
  // Check if this is a synthetic message (generated by bridge service)
  const isSynthetic = (message as any).metadata?.isSynthetic || 
                      messageId.startsWith('status_') ||
                      /^(Searching the web for|Reading .+\.\.\.|Editing .+\.\.\.|Writing to .+\.\.\.|Executing: |Running agent task:|Using .+\.\.\.)/.test(content);

  // Debug logging for synthetic message detection
  if (content && (messageId.startsWith('status_') || content.includes('Searching the web'))) {
    console.log('🔍 AssistantMessage synthetic detection:', {
      messageId,
      content: content.substring(0, 50),
      isSynthetic,
      metadata: (message as any).metadata,
      startsWithStatus: messageId.startsWith('status_'),
      contentMatches: /^(Searching the web for|Reading .+\.\.\.|Editing .+\.\.\.|Writing to .+\.\.\.|Executing: |Running agent task:|Using .+\.\.\.)/.test(content)
    });
  }
  ...
  // Render synthetic message with loader effect
  if (isSynthetic) {
    // Extract tool name from content for better display
    let toolName = '';
    if (content.includes('Searching the web')) toolName = 'WebSearch';
    else if (content.includes('Reading ')) toolName = 'Read';
    else if (content.includes('Editing ')) toolName = 'Edit';
    else if (content.includes('Writing to ')) toolName = 'Write';
    else if (content.includes('Executing:')) toolName = 'Bash';
    else if (content.includes('Running agent task:')) toolName = 'Task';
    else if (content.includes('Using ')) {
      const match = content.match(/Using ([^.]+)\.\.\./);
      toolName = match ? match[1] : 'Tool';
    }

    return (
      <SyntheticMessageLoader
        content={content}
        toolName={toolName}
        timestamp={timestamp}
        isVisible={true}
