 ...
 * - Always-expanded plan content with markdown formatting
 * - Interactive approval/denial buttons
 * - Plan editing and feedback capabilities
...
const ExitPlanModeMessage: React.FC<ExitPlanModeMessageProps> = ({
  ...
  const title = "Implementation Plan Review";
  const description = "Claude Code has generated an implementation plan for your review.";
  ...
  const handleSubmit = useCallback((action?: PlanReviewDecision['decision']) => {
    ...
    // TODO: Implement actual decision handling - likely through bridge API
  ...
  }, [selectedAction, feedback, isEditMode, editedPlan, planContent]);
  ...
  const shortMsg = description.length > 160 ? description.slice(0, 160) + '…' : description;
